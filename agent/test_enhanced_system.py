...
async def test_enhanced_agent():
    ...
    try:
        ...
        print(f"   ✅ 告警测试成功")
        print(f"   🚨 告警动作: {alert_result.get('action_triggered', 'None')}")
        ...
        return True, {
            ...
            'alert_case_result': alert_result
        }
...
def test_individual_components():
    ...
    try:
        ...
        knowledge_file = Path('data/test_knowledge.json')
        ...
        test_results['vector_rag'] = {
            ...
            'results_count': len(results),
            'details': '向量化RAG工具运行正常' if results else '使用降级文本匹配'
        }
        
        print(f"   ✅ 向量化RAG: {test_results['vector_rag']['details']}")
    ...
    try:
        ...
        print(f"   ✅ Prometheus监控: 功能正常，运行时间{stats['uptime_formatted']}")
...
async def main():
    ...
    # 测试完整Agent
    print("\n🚀 第二阶段：增强Agent集成测试")
    agent_success, agent_data = await test_enhanced_agent()
    ...
    final_report = {
        'test_summary': {
            ...
            'agent_integration_success': agent_success,
    ...
    report_file = 'data/outputs/final_test_report.json'
